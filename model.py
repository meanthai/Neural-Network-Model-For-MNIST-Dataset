...
def PCA(X):
    ...
    for ite, number in enumerate(cum_var_exp):
        if number >= 95:
            num_components = ite
            print(ite)
            break
...
batch_size = 4
mlp = MLP(input_size=X.shape[1], output_size=num_classes, hidden_sizes = [284, 128, 16], alpha= 0.01, learning_rate=0.001, epochs=15, batch = batch_size, tol = 0.000001)
